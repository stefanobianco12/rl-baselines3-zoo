...
def projection_L_2(delta,eps):
    return (eps*delta/(norms(delta).clamp(min=eps)))
...
def pgd_l2(model, X, y, epsilon):
    ...
    for i in range(niters):
        ...
        loss.backward()
        eta = step_size * X_adv.grad.data/norms(X_adv.grad.data)
        X_adv.data = X_adv.data + eta
        X_adv=X_adv.clone().detach().requires_grad_(True)
        # adjust to be within [-epsilon, epsilon]

        eta = projection_L_2(X_adv.data-X_tensor.data,epsilon)
...
def enjoy() -> None:  # noqa: C901
    ...
    parser.add_argument("--eps", help="Epsilon", default=0, type=float)
    parser.add_argument("--frq", help="frequency", default=1, type=int)
    parser.add_argument("--beta", help="beta", default=0, type=float)
   ...
   #         raise ImportError("Please install tqdm and rich to use the progress bar")
    ...
    print(f"Mean reward: {np.mean(ep_rew):.2f} +/- {np.std(ep_rew):.2f}")
